   ...
   for (int c = 0; c < n; c++)
    ...
    for (int final = 0; final < n*n*5000; final++)
    {
        ...
        if (lamda > 1)
        {
            for (count = n-2; count > 1; count--)
            {
                for (int i = 0; i < n - count + 1; i++)
                {
                    ...
                    {
                        mu = count/n;
                        base = n*n*5*pow((1/lamda), 1)*(exp(-pow((mu - 0.4), 2)/0.3)/10 - 0.06);
                        if (base < 0)
                        {
                            base = 0;
                        }
                        ...
                        if (base != 0)
                        {
                            super[ant[i]][ant[i+1]][1] = super[ant[i]][ant[i+1]][1] + base;
                            super[ant[i+1]][ant[i]][1] = super[ant[i+1]][ant[i]][1] + base;
                            for (int b = 1; b < count-1; b++)
                            {
                                super[ant[i+b]][ant[i+b+1]][1] = super[ant[i+b]][ant[i+b+1]][1] + pow(0.8 + atan((count - 2)/50)/16, b)*base;
                                super[ant[i+b+1]][ant[i+b]][1] = super[ant[i+b+1]][ant[i+b]][1] + pow(0.8 + atan((count - 2)/50)/16, b)*base;
                            }
        ...
        if (lamda < 1) 
        {
            ...
            double base = n*n*5*pow((1/lamda), 3);
            super[ant[n/2-1]][ant[n/2]][1] = super[ant[n/2-1]][ant[n/2]][1] + ceil(base*100)/100;
            super[ant[n/2]][ant[n/2-1]][1] = super[ant[n/2]][ant[n/2-1]][1] + ceil(base*100)/100;
            for (int i = 1; i < n/2; i++)
            {
                super[ant[n/2-i-1]][ant[n/2-i]][1] = super[ant[n/2-i-1]][ant[n/2-i]][1] + ceil(pow(0.85, i)*base*100)/100;
                super[ant[n/2-i]][ant[n/2-i-1]][1] = super[ant[n/2-i]][ant[n/2-i-1]][1] + ceil(pow(0.85, i)*base*100)/100;
                super[ant[n/2+i-1]][ant[n/2+i]][1] = super[ant[n/2+i-1]][ant[n/2+i]][1] + ceil(pow(0.85, i)*base*100)/100;
                super[ant[n/2+i]][ant[n/2+i-1]][1] = super[ant[n/2+i]][ant[n/2+i-1]][1] + ceil(pow(0.85, i)*base*100)/100;
            }
